...
import { X, Crown, Star, Zap, Check, Sparkles, IndianRupee, Loader2 } from 'lucide-react';
...
export const PremiumModal: React.FC<PremiumModalProps> = ({
  ...
  const handleUpgrade = async (tierId: string) => {
    ...
    try {
      const result = await upgradeToPremium(tierId);
  ...
  return (
    <>
                    ...
                    {SUBSCRIPTION_TIERS.map((tier) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          disabled={subscription?.tier === tier.id || isUpgrading || tier.price === 0}
                          className={`w-full py-3 rounded-lg font-mono transition-all flex items-center justify-center space-x-2 ${
                        ...
                        >
                          {isUpgrading && upgradingTier === tier.id ? (
                            <>
      ...
      <UpgradeSuccessModal
        ...
        unlockedFeatures={upgradeResult?.unlockedFeatures || []}
      />
    </>
  );
};